...
void Camera::save_preset(uint8_t number) {
    send_command({0x01, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x1e, 0x81, 0x01, 0x04, 0x3F, 0x01, number, 0xFF});
}
...
void Camera::rotate(double pan, double tilt) {
    ...
    send_command({0x01, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x1e, 0x81,0x01,0x06,0x01,pan_power,tilt_power,pan_direction,tilt_direction,0xFF});
}
...
void Camera::zoom(double speed) {
    ...
    send_command({0x01, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x1e, 0x81, 0x01, 0x04, 0x07, direction, 0xFF});
}
